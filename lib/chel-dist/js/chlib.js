...
function renderTextInsideRectangle(canvas, context, _text, _x1, _y1, _w, _h, _font, _textColor, _lineHeight)
{
	var splitedText = splitTextIntoLinesWithMeasurement(canvas, context, _text, _x1, _y1, _w, _h, _font);
	renderSplitedText(canvas, context, splitedText, _x1, _y1, _w, _h, _font, _textColor, _lineHeight);
}

function splitTextIntoLinesWithMeasurement(canvas, context, _text, _x1, _y1, _w, _h, _font)
{
	...
	var mMeasure = context.measureText("m").width;
	var char = ""; 
	var charIndex = 0;
	var firstSpaceIndex = 0;
	var lastSpeceIndex = 0;
	var lineText = ""; 
	var text = _text.trim();
	var printLine = false;
	var counter = 0;
	...
	{
		counter = counter + 1;
		char = text.substring(charIndex, charIndex + 1);

		lineText = lineText + char;
		if (" ".localeCompare(char) === 0)
			lastSpeceIndex = charIndex;
		...
		if (context.measureText(lineText).width >= (_w - mMeasure))
		{
			skeepSpace = 1;
			if (firstSpaceIndex === lastSpeceIndex)
			{
				skeepSpace = 0;
				lastSpeceIndex = charIndex;
			}
		...
		if (printLine === true)
		{
			lineText = text.substring(firstSpaceIndex, lastSpeceIndex);
			resultLines.push(lineText);

			firstSpaceIndex = lastSpeceIndex + skeepSpace;
			charIndex = lastSpeceIndex + skeepSpace; 
		...
		charIndex++;
	} while (charIndex <= _text.length && counter < 1000)
	context.restore();

	return resultLines;
}

function renderSplitedText(canvas, context, _splitedText, _x1, _y1, _w, _h, _font, _textColor, _lineHeight)
{
	context.save();
	...
	*/
	...
	for (var index = 0; index < _splitedText.length; index++) 
	{
		context.fillText(_splitedText[index], _x1, _y1 + (_lineHeight * (index + 1)));
	}
