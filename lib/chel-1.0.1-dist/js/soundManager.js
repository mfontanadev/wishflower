// Class SoundManager:
//    Se encarga de cargar dinamicamente los sonidos y gestionar su utilizacion 
//    bajo demanda de las clases que necesiten dibujar.

// TODO: test not loaded plugin case
// TODO: test empty file array
...
function SoundManager()
{
	...
	{
		msglog('INIT SOUND:initializaSoundPlugin');

		this.m_managerAvailable = createjs.Sound.initializeDefaultPlugins();

		if (this.m_managerAvailable === false)
		{
			msglog('INIT SOUND ERROR: createjs.Sound.initializeDefaultPlugins() not created.');
        }
        else
        {
			msglog('INIT SOUND SUCCESS: new createjs.Sound.initializeDefaultPlugins() created.');
        }
	};
	...
	SoundManager.prototype.setOnSuccessEventListener = function (_callback)
	{
	...
	SoundManager.prototype.performAsynchLoadBase = function(arr, _callback, _owner)
	{
		...
		else
		{
			...
			{
				msglog('SoundManager resource loaded:' + event.id);
				updateProgressBar();
			};

			function onError(event)
			{
				msglog('ERROR: SoundManager resource loaded:' + event.id);
				_owner.m_managerAvailable = false;
				updateProgressBar();
			};
	...
	{ 
		...
		{
			console.log(SoundManager.self.m_idProgressBar._visible);
